...
public client class Client {
    ...
    remote function getCustomer(string customerId) returns @untainted json|error {
        ...
        // Query | DB to stream
        stream<record { }, error> rs = self.jdbcClient->query(
        `SELECT * FROM Customer WHERE Id=${<@untainted>customerId}`);
    ...
    remote function addCustomer(Customer customer) returns string|error? {
        var result = self.jdbcClient->execute(`INSERT INTO Customer(name, country, email) VALUES(${<@untainted>customer.
        name},${<@untainted>customer.country},${<@untainted>customer?.email})`);
    ...
    remote function updateCustomer(Customer customer, string customerId) returns string|error? {
        ...
        var result = self.jdbcClient->execute(`UPDATE Customer SET name=${<@untainted>customer.name}, country=${<@untainted>
        customer.country}, email=${<@untainted>customer.email} WHERE Id = ${<@untainted>customerId}`);
