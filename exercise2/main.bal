    ...
    resource function post customer(http:Caller caller, http:Request request) returns @untainted error? {
        ...
        if (payload is json) {
            if (payload is ()) {
                ...
                _ = check dbClient->execute(`INSERT INTO Customer(Name, Country, Email) VALUES(${<@untainted><string>
                cusName},${<@untainted><string>cusCountry},${<@untainted><string>cusEmail})`);

                res.statusCode = 201;
                res.setJsonPayload({
                    status: "Successfully Saved",
                    payload: <@untainted>payload
                });
    ...
    resource function put customer/[int _id](http:Caller caller, http:Request request) {
        ...
        if (payload is json) {
            if (payload is ()) {
                ...
                var objj = dbClient->execute(`UPDATE Customer SET Name = ${<@untainted><string>cusName} , Email = ${<@untainted><string>
                cusEmail}, Country = ${<@untainted><string>cusCountry} WHERE Id = ${_id.toString()} `);
